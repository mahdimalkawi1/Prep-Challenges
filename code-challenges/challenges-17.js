...
const recursionPattern = (int1, int2) => {
    ...
    const recursiveHelper = (num) => {
      ...
      pattern.push(pattern[pattern.length - 1] - int2);
      recursiveHelper(num - 1);
      if (num !== 1) {
        pattern.push(pattern[pattern.length - 1] + int2);
      }
    };
    recursiveHelper(Math.ceil(int1 / int2));
    pattern[pattern.length]=pattern[pattern.length-1]+int2
    return pattern;
...
const filterLinks = (str) => {
    // write your code here
    let match=str.match(/href="(http:\/\/)?([^"]+\.(com|org|net))"/);
